...
export const useAudienceStats = ({ roomId, currentSlide, isPresenterWsReady }: { roomId?: any; currentSlide?: any; isPresenterWsReady?: any }) => {
  ...
  useEffect(() => {
    ...
    const loadAudienceStats = async () => {
      try {
        ...
        if (audienceStatsControllerRef.current === controller) audienceStatsControllerRef.current = null;
      }
    };
