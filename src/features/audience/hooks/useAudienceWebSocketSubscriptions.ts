...
const useAudienceWebSocketSubscriptions = ({
  ...
  useEffect(() => {
    ...
    websocketService.connect(wsUrl, audienceToken, () => setIsWebsocketReady(true), () => setIsWebsocketReady(false));
  ...
  useEffect(() => {
    ...
    pageChangeUnsubscribeRef.current = websocketService.subscribe(
      ...
      (data: any) => {
        ...
        changeCurrentSlide(newSlideIndex, { source: "presenter", broadcast: false, preserveFollowState: true });
      }
    ...
    sessionStateUnsubscribeRef.current = websocketService.subscribe(
      ...
      (data: any) => {
        ...
        if (data && data.type === "SESSION_STATE" && data.status) {
          ...
          if ((data.status === "ended" || data.status === "ENDED") && !hasNavigatedToRatingRef.current) {
    ...
    isWebsocketReady, roomId, audienceId, deckId,
    handleIncomingQuestion, questionTopics,
    changeCurrentSlide, currentSlide,
    followPresenterRef, setFollowPresenter, lastPresenterPageRef,
    setSessionStatus, setQuickSettings, setUnlockSettings,
    code, codeParam, navigate,
  ]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = websocketService.subscribeText(
      ...
      (rawMessage) => {
        ...
        triggerFocusHighlight();
        changeCurrentSlide(parsedIndex, { source: "focusOn", broadcast: true, preserveFollowState: true });
      }
    );
  ...
  }, [isWebsocketReady, roomId, changeCurrentSlide, followPresenterRef, setFollowPresenter, lastPresenterPageRef, triggerFocusHighlight]);
