...
const useAudienceJoinRoom = ({
  ...
  setFollowPresenter,
  changeCurrentSlide,
}: { code?: any; lastPresenterPageRef?: any; setFollowPresenter?: any; changeCurrentSlide?: any }) => {
  ...
  useEffect(() => {
      ...
      if (stored) {
        ...
        if (storedData.audienceId && storedData.audienceToken) {
          ...
          if (storedData.sticker != null) {
            setQuickSettings((prev) => ({ ...prev, sticker: String(storedData.sticker) === "true" }));
          }
          if (storedData.question != null) {
            setQuickSettings((prev) => ({ ...prev, question: String(storedData.question) === "true" }));
          }
          if (storedData.feedback != null) {
            setQuickSettings((prev) => ({ ...prev, feedback: String(storedData.feedback) === "true" }));
          }
          ...
          if (storedData.maxPage !== undefined || storedData.slideUnlock !== undefined) {
            ...
            const slideUnlock = storedData.slideUnlock ? String(storedData.slideUnlock) === "true" : true;
            setUnlockSettings({
          ...
          if (storedData.roomId) {
            const syncRoomInfo = async () => {
              try {
                ...
                if (roomInfo.currentPage) {
                  ...
                  if (Number.isFinite(presenterPage) && presenterPage > 0) {
                    ...
                    if (changeCurrentSlide && typeof changeCurrentSlide === "function") {
                      changeCurrentSlide(presenterIndex, { source: "presenter", broadcast: false, preserveFollowState: true });
                    }
                ...
                if (roomInfo.sticker != null) setQuickSettings((prev) => ({ ...prev, sticker: String(roomInfo.sticker) === "true" }));
                if (roomInfo.question != null) setQuickSettings((prev) => ({ ...prev, question: String(roomInfo.question) === "true" }));
                if (roomInfo.feedback != null) setQuickSettings((prev) => ({ ...prev, feedback: String(roomInfo.feedback) === "true" }));

                if (roomInfo.maxPage !== undefined || roomInfo.slideUnlock !== undefined) {
                  const maxPage = roomInfo.maxPage ? Number(roomInfo.maxPage) : null;
                  const slideUnlock = roomInfo.slideUnlock ? String(roomInfo.slideUnlock) === "true" : true;
                  setUnlockSettings({
                    maxRevealedPage: maxPage,
                    revealAllSlides: slideUnlock,
    ...
    const handleJoinRoom = async () => {
      try {
          ...
          const dataToStore = {
            ...
            wsUrl: (() => {
              ...
              if (!wsUrlValue) {
                ...
                if (!wsUrlValue.endsWith("/audience")) wsUrlValue = wsUrlValue.replace(/\/ws\/?$/, "/ws/audience");
              }
              return wsUrlValue;
            })(),
          };
        ...
        const resolvedDeckId = joinData.deckId || joinData.deckID || joinData.deck?.deckId || joinData.presentation?.deckId;
        if (resolvedDeckId) setDeckId(resolvedDeckId);

        const resolvedTotalPages = joinData.totalPages || joinData.deck?.totalPages || joinData.presentation?.totalPages;
        ...
        if (joinData.sticker != null) setQuickSettings((prev) => ({ ...prev, sticker: String(joinData.sticker) === "true" }));
        if (joinData.question != null) setQuickSettings((prev) => ({ ...prev, question: String(joinData.question) === "true" }));
        if (joinData.feedback != null) setQuickSettings((prev) => ({ ...prev, feedback: String(joinData.feedback) === "true" }));
